<script lang="ts">
  ...
  import {
    getStreak, getAvgAccuracy,
    getNoteTendencies, getRecentSessions,
    getDailyAggregates, getSessionCount, getTotalMinutes,
  ...
  let aggregates = $derived(getDailyAggregates(period));
  let sessionCount = $derived(getSessionCount(period));
  let totalMinutes = $derived(getTotalMinutes(period));
  let avgAccuracy = $derived(getAvgAccuracy(period ?? 9999));

  // Chart series extracted from aggregates
  let accuracyData = $derived(
    aggregates.filter(d => d.sessions > 0).map(d => d.accuracy),
  );
...
<div class="progress-view">
  ...
  <div class="overview-row">
    ...
    <div class="card overview-card">
      <div class="ov-value">{totalMinutes}<span class="ov-unit">{$t('progress.minutes_short')}</span></div>
