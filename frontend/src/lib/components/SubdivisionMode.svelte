<script lang="ts">
  ...
  import {
    subPhase, subType, subBpm, subBars, subTimeSignature,
    countdownBeat, barResults, overallEvenness,
    startSubdivisionExercise, stopSubdivisionExercise,
    ...
    <div class="sub-content">
      ...
      <div class="sub-sidebar">
        ...
        <div class="setting-group">
          ...
          <div class="pill-row">
            {#each subTypes as st}
              <button class="pill" class:active={$subType === st.key} onclick={() => subType.set(st.key)}>
                {$t(st.label)}
              </button>
            {/each}
          </div>
        ...
        <div class="setting-group">
          ...
          <div class="pill-row">
            {#each timeSigs as ts}
              <button class="pill" class:active={$subTimeSignature === ts} onclick={() => subTimeSignature.set(ts)}>
                {ts}
              </button>
            {/each}
          </div>
      ...
      <div class="sub-main">
        ...
        {#each $barResults as bar}
          <div class="bar-result">
            <div class="bar-header">{$t('rhythm.sub_trainer.bar')} {bar.barNumber} — {bar.evennessScore} ms RMS</div>
              ...
              {#each bar.slots as slot}
                <div class="slot-col" class:main-beat={slot.isMainBeat}>
                  ...
                  {#if slot.deviationMs !== null && !slot.isMainBeat}
                    ...
                    >{slot.deviationMs >= 0 ? '+' : ''}{slot.deviationMs}</div>
                  {:else if !slot.isMainBeat}
        ...
        <div class="scoring-controls">
          <button class="ctrl-btn" onclick={stopSubdivisionExercise}>{$t('rhythm.sub_trainer.stop')}</button>
          <button class="ctrl-btn accent" onclick={() => { stopSubdivisionExercise(); startSubdivisionExercise(); }}>
...
<style>
  ...
  /* ── Preview / BPM ── */
  .sub-preview { display: flex; flex-direction: column; align-items: center; gap: 16px; }
  .sub-hint { font-size: 13px; color: var(--text-3); text-align: center; max-width: 320px; line-height: 1.5; margin: 0; }

  .bpm-area { display: flex; align-items: center; gap: 20px; }
  .bpm-btn {
    width: 36px; height: 36px; border-radius: 50%; border: 1px solid var(--border);
    background: var(--surface); color: var(--text); font-size: 18px; font-weight: 600;
    cursor: pointer; display: flex; align-items: center; justify-content: center;
    font-family: inherit; transition: all 0.12s;
  }
  .bpm-btn:hover { background: var(--surface-hover); }
  .bpm-display { text-align: center; }
  .bpm-value { font-size: 42px; font-weight: 900; letter-spacing: -2px; line-height: 1; color: var(--text); }
  .bpm-label { font-size: 10px; color: var(--text-3); text-transform: uppercase; letter-spacing: 2px; display: block; margin-top: 2px; }
  .bpm-slider { width: 220px; accent-color: var(--accent); cursor: pointer; }
  ...
  .countdown-num { font-size: 72px; font-weight: 900; color: var(--text); animation: pop 0.3s ease-out; }
  ...
  @keyframes pulse-play { from { transform: scale(1); opacity: 0.6; } to { transform: scale(1.3); opacity: 1; } }
  .playing-label { font-size: 18px; font-weight: 700; color: var(--text); }

  /* ── Evenness display ── */
  .evenness-display { display: flex; flex-direction: column; align-items: center; gap: 2px; }
  .evenness-value { font-size: 48px; font-weight: 900; letter-spacing: -2px; }
  .evenness-value.good { color: var(--green); }
  .evenness-value.warn { color: var(--amber); }
  .evenness-value.bad { color: var(--red); }
  .evenness-unit { font-size: 11px; color: var(--text-3); }
  .evenness-label { font-size: 10px; color: var(--text-3); text-transform: uppercase; letter-spacing: 1px; }
  ...
  .bar-header { font-size: 11px; color: var(--text-3); font-weight: 600; margin-bottom: 8px; }

  /* ── Deviation chart ── */
  .deviation-chart { display: flex; gap: 2px; }

  .slot-col {
    flex: 1; display: flex; flex-direction: column; align-items: center; gap: 2px;
  }
  .slot-col.main-beat { opacity: 0.4; }
  ...
  .dev-dot.good { background: var(--green); }
  .dev-dot.warn { background: var(--amber); }
  .dev-miss {
    position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);
    font-size: 10px; color: var(--red); font-weight: 700;
  }

  .slot-label { font-size: 9px; color: var(--text-2); font-weight: 600; white-space: nowrap; }
  .slot-label.sub-label { color: var(--text-3); }

  .slot-dev { font-size: 8px; font-weight: 600; font-variant-numeric: tabular-nums; }
  .slot-dev.good { color: var(--green); }
  .slot-dev.warn { color: var(--amber); }
  .slot-dev.miss { color: var(--red); }
