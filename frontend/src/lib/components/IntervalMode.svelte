<script lang="ts">
  ...
  import {
    ...
    intervalChallenge,
    intervalResults,
    intervalHoldProgress,
    intervalCurrentCents,
...
<div class="interval-panel">
  {#if $intervalPhase === 'idle'}
    <div class="interval-prompt">
      ...
      {:else}
        <p class="interval-helper">{$t('tonelab.activate_hint')}</p>
      {/if}
    </div>
  {:else if $intervalChallenge}
    ...
    <div class="interval-challenge">
      <div class="iv-root">
        ...
        <div class="iv-note">
          <PracticeNote
            note={$intervalChallenge.rootNote}
            octave={$intervalChallenge.rootOctave}
            size="md"
            sourceMode="concert"
          />
        </div>
      ...
      <div class="iv-target">
        ...
        <div class="iv-note target-note">
          <PracticeNote
            note={$intervalChallenge.targetNote}
            octave={$intervalChallenge.targetOctave}
            size="md"
            sourceMode="concert"
            accent
          />
    ...
    <div class="iv-phase">
      ...
      {:else if $intervalPhase === 'playing'}
        ...
        <div class="hold-bar-wrap">
          <div
            ...
            class:warn={Math.abs($intervalCurrentCents) > 5 &&
              Math.abs($intervalCurrentCents) <= 15}
          ></div>
        </div>
      ...
      {:else if $intervalPhase === 'result'}
        ...
        {:else}
          <span class="phase-text warn"
            >{fmtCents($intervalResults[$intervalResults.length - 1]?.avgCents ?? 0)}ct</span
          >
        {/if}
      {/if}
    </div>
    ...
    {#if $intervalPhase === 'result'}
      <div class="iv-actions">
        ...
        <button class="iv-stop-btn" onclick={stopIntervalTraining}
