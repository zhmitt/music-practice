<script lang="ts">
  ...
  import { generateSmartSuggestion } from '$lib/exercises/smartSuggestion';
  ...
  let pitchNote = $state('--');
  let pitchNoteName = $state('Bb');
  let pitchOctave = $state(4);
  let pitchHz = $state('---');
  let pitchCents = $state(0);
  let stableTime = $state(0);
  let lastStableSampleAt = 0;
  let onboardingLease: AudioLeaseHandle | null = null;
  let leaseGeneration = 0;
  let inputLevel = $state(0);
  let signalDetected = $state(false);
  let pitchDetected = $state(false);
  let clippingDetected = $state(false);
  ...
  const experiences: { id: ExperienceLevel; key: string; subKey: string }[] = [
    {
      id: 'beginner_new',
      key: 'ob.experience.beginner_new',
      subKey: 'ob.experience.beginner_new_sub',
    },
    { id: 'beginner', key: 'ob.experience.beginner', subKey: 'ob.experience.beginner_sub' },
    {
      id: 'intermediate',
      key: 'ob.experience.intermediate',
      subKey: 'ob.experience.intermediate_sub',
    },
    {
      id: 'experienced',
      key: 'ob.experience.experienced',
      subKey: 'ob.experience.experienced_sub',
    },
  ];
  ...
  function resetMicFeedback() {
    ...
    pitchCents = 0;
    stableTime = 0;
    lastStableSampleAt = 0;
    inputLevel = 0;
    signalDetected = false;
    pitchDetected = false;
    clippingDetected = false;
    toneDetected.set(false);
  }
  ...
  async function requestMicrophone() {
    ...
    if (!onboardingLease) {
      const acquiredLease = await acquireAudioLease('onboarding', {
  ...
  async function handleMicrophoneChange(event: Event) {
    ...
    // setSelectedMicrophoneDevice coordinates a restart for all active leases.
  }
  ...
  function startPitchPolling() {
    ...
    const poll = async () => {
      ...
      try {
        const [pitch, level] = await Promise.all([
          invokeTauri('get_pitch'),
          invokeTauri('get_audio_level'),
        ]);

        inputLevel = level?.rms ?? 0;
        signalDetected = inputLevel >= 0.02;
        clippingDetected = Boolean(level?.is_clipping);

        if (pitch) {
          pitchDetected = true;
          pitchNote = `${pitch.note_name}${pitch.octave}`;
          pitchNoteName = pitch.note_name;
          pitchOctave = pitch.octave;
          pitchHz = `${pitch.frequency_hz.toFixed(1)} Hz`;
          pitchCents = Math.round(pitch.cent_deviation);

          // Track stability: within +/-20 cents and a confident detection
          if (pitch.confidence >= 0.5 && Math.abs(pitch.cent_deviation) <= 20) {
            const now = performance.now();
            stableTime += lastStableSampleAt === 0 ? 0 : Math.min(now - lastStableSampleAt, 200);
            lastStableSampleAt = now;
            if (stableTime >= 1000) {
        ...
        if (generation === pollingGeneration) {
          pollInterval = setTimeout(() => void poll(), 75);
        }
      }
    };
    void poll();
  ...
  async function finish() {
    ...
    const suggestion = generateSmartSuggestion(profile);
  ...
  let microphoneStatusKey = $derived.by(() => {
    if (!$micGranted) return 'ob.mic.text';
    if ($toneDetected) return 'ob.mic.status_ready';
    if (clippingDetected) return 'ob.mic.clipping';
    if (pitchDetected) return 'ob.mic.status_pitch';
    if (signalDetected) return 'ob.mic.status_signal';
    return 'ob.mic.status_no_signal';
  });

  let microphoneStatusTone = $derived.by(() => {
    if ($toneDetected) return 'good';
    if (pitchDetected || signalDetected || clippingDetected) return 'warn';
    return 'neutral';
  });

  let onboardingCtaKey = $derived.by(() =>
    $toneDetected ? 'ob.mic.start_session' : 'ob.mic.continue_anyway',
  );
</script>
              ...
              >
                <div class="instrument-icon">
                  <svg viewBox="0 0 28 28"
                    ><path d="M4 14c0-5.52 4.48-10 10-10s10 4.48 10 10-4.48 10-10 10" /><circle
                      cx="14"
                      cy="14"
                      r="3"
                    /><path d="M14 4v4M14 18v2M4 14h4M20 14h4" /></svg
                  >
                </div>
            ...
            <div class="ob-pill-group">
              {#each dayOptions as d}
                <button
                  ...
                  onclick={() => selectedDays.set(d)}>{d}</button
                >
              {/each}
            </div>
          </div>
            ...
            <div class="ob-pill-group">
              {#each minuteOptions as m}
                <button
                  ...
                  onclick={() => selectedMinutes.set(m)}>{m}</button
                >
              {/each}
            </div>
          </div>
      ...
      {#if $onboardingStep === 5}
        <div class="ob-screen">
          ...
          <div class="ob-mic-section">
            <div class="ob-device-row">
              <label class="ob-device-label" for="onboarding-microphone"
                >{$t('settings.microphone')}</label
              >
              <div class="ob-device-controls">
                <select
                  id="onboarding-microphone"
                  class="ob-device-select"
            ...
            {:else}
              ...
              <div
                class="ob-audio-status"
                class:good={microphoneStatusTone === 'good'}
                class:warn={microphoneStatusTone === 'warn'}
              >
                {$t(microphoneStatusKey)}
              </div>
              <div class="ob-level-row" aria-hidden="true">
                {#each Array(5) as _, i}
                  <div class="ob-level-bar" class:active={inputLevel > (i + 1) * 0.03}></div>
                {/each}
              </div>
              <div class="ob-status-grid">
                <div class="ob-status-pill" class:done={signalDetected}>
                  {$t('ob.mic.signal_label')}
                </div>
                <div class="ob-status-pill" class:done={pitchDetected}>
                  {$t('ob.mic.pitch_label')}
                </div>
                <div class="ob-status-pill" class:done={$toneDetected}>
                  {$t('ob.mic.ready_label')}
                </div>
              </div>
              <div class="ob-tone-section">
                ...
                <div class="ob-tone-result">
                  ...
                  <div class="ob-pitch-meter">
                    ...
                    <div
                      class="ob-pitch-dot"
                      class:inactive={!pitchDetected}
                      style="top: {dotPosition(pitchCents)}%"
                    ></div>
                  </div>
                ...
                {:else if pitchDetected}
                  <div class="ob-helper">{$t('ob.mic.status_pitch')}</div>
                {:else if signalDetected}
                  <div class="ob-helper">{$t('ob.mic.status_signal')}</div>
                {:else}
                  <div class="ob-helper">{$t('ob.mic.status_no_signal')}</div>
                {/if}
              </div>
            {/if}
          </div>
          ...
          <button
            class="ob-cta"
            class:secondary={!$toneDetected}
            class:disabled={!$micGranted}
            onclick={finish}
            disabled={!$micGranted}
          >
            {$t(onboardingCtaKey)}
...
<style>
  ...
  .instrument-card.selected .instrument-icon svg {
    stroke: var(--accent-2);
  }
  .instrument-name {
    font-size: 12px;
  ...
  .experience-option.selected .exp-radio {
    border-color: var(--accent);
  }
  .experience-option.selected .exp-radio::after {
    content: '';
    width: 8px;
    height: 8px;
    border-radius: 50%;
  ...
  .experience-option.selected .exp-label {
    color: var(--accent-2);
  }
  ...
  .ob-audio-status.good {
    background: var(--green-soft);
    color: var(--green);
    border-color: rgba(52, 211, 153, 0.18);
  }
  .ob-audio-status.warn {
