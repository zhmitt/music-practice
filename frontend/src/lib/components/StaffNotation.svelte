...
<div class="staff-wrap">
  <svg viewBox="0 0 {svgWidth} 100" width={svgWidth} height="100" class="staff-svg">
    ...
    <text x="16" y={staffY(0) + 8} font-size="32" fill="var(--text-3)" font-family="serif"
      >&#119070;</text
    >
    ...
    {#each notePositions as np}
      ...
      {#if np.acc}
        <text
          ...
          y={np.y + 4}
          font-size="12"
          fill={np.index === currentIndex
            ? 'var(--accent)'
            : np.index < currentIndex
              ? 'var(--green)'
              : 'var(--text-3)'}
          text-anchor="middle"
          font-family="serif">{np.acc === 'b' ? '\u266D' : '\u266F'}</text
        >
      {/if}
      ...
      {#if np.pos >= 0}
        ...
        <line
          x1={np.x - NOTE_RADIUS + 0.5}
          y1={np.y}
          x2={np.x - NOTE_RADIUS + 0.5}
          y2={np.y + 24}
          stroke={np.index === currentIndex
            ? 'var(--accent)'
            : np.index < currentIndex
              ? 'var(--green)'
              : 'var(--text-3)'}
