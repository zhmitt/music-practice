<script lang="ts">
  ...
  import { ALL_PATTERNS, getPatternsByLevel, type PatternLevel, type RhythmPattern } from '$lib/rhythm/patterns';
  import {
    patternPhase, currentPattern, currentRound, roundResults, latestTimings, countdownBeat,
    startPatternSession, repeatRound, nextRound, stopPatternSession,
  ...
  let selectedPattern = $derived(ALL_PATTERNS.find(p => p.id === selectedPatternId) ?? ALL_PATTERNS[0]);
    ...
    <div class="pattern-content">
      <div class="pattern-main">
        <div class="pattern-preview">
          ...
          <div class="preview-info">{selectedPattern.timeSignature} / {selectedPattern.bpm} BPM</div>
  ...
  {:else}
    ...
    <div class="pattern-content">
      <div class="pattern-main">
        <!-- Round indicator -->
        <div class="round-row">
          <span class="round-text">{$t('rhythm.pat.round')} {$currentRound} {$t('rhythm.pat.of')} 4</span>
          <div class="round-dots">
            {#each [1, 2, 3, 4] as r}
              <span class="round-dot" class:filled={r <= $currentRound} class:current={r === $currentRound}></span>
            {/each}
          </div>
        </div>
        ...
        {:else if $patternPhase === 'scoring'}
          ...
          <div class="scoring-area">
                ...
                <div class="timing-row">
                  ...
                  <span class="timing-icon">
                    {#if timing.actualMs === null}
                      <svg viewBox="0 0 16 16" width="14" height="14"><line x1="4" y1="4" x2="12" y2="12" stroke="var(--red)" stroke-width="2"/><line x1="12" y1="4" x2="4" y2="12" stroke="var(--red)" stroke-width="2"/></svg>
                    {:else if timing.hit}
                      <svg viewBox="0 0 16 16" width="14" height="14"><path d="M4 8l3 3 5-5" stroke="var(--green)" stroke-width="2" fill="none"/></svg>
                    {:else}
                      <svg viewBox="0 0 16 16" width="14" height="14"><circle cx="8" cy="8" r="5" stroke="var(--amber)" stroke-width="2" fill="none"/></svg>
                    {/if}
                  </span>
                  <span class="timing-dev"
                    class:good={timing.hit}
                    class:warn={timing.deviationMs !== null && !timing.hit}
                    class:miss={timing.actualMs === null}
                  >
                    {timing.actualMs === null ? $t('rhythm.pat.missed') : `${fmtDev(timing.deviationMs)} ms`}
                  </span>
                </div>
              {/each}
            </div>
            ...
            <div class="scoring-controls">
              ...
              {#if !isComplete}
                <button class="ctrl-btn accent" onclick={nextRound}>{$t('rhythm.pat.weiter')}</button>
              {:else}
                <button class="ctrl-btn accent" onclick={stopPatternSession}>{$t('session.done')}</button>
              {/if}
            </div>
          </div>
        {/if}
      ...
      <div class="pattern-sidebar">
        {#if $currentPattern}
          <div class="setting-group">
            ...
            <div class="pat-task-meta">{$currentPattern.timeSignature} / {$currentPattern.bpm} BPM</div>
          </div>
          <div class="setting-group">
...
<style>
  ...
  .setting-label { font-size: 10px; color: var(--text-3); text-transform: uppercase; letter-spacing: 1px; }
  ...
  .pill.active { background: var(--accent-soft); color: var(--text); border-color: var(--accent); font-weight: 600; }
  ...
  .pattern-item {
    ...
    background: transparent; color: var(--text-2); font-family: inherit;
    font-size: 13px; cursor: pointer; transition: all 0.12s; text-align: left;
  }
  .pattern-item:hover { background: var(--surface-hover); color: var(--text); }
  .pattern-item.active { background: var(--accent-soft); color: var(--text); border-color: var(--accent); font-weight: 600; }
  .pat-meta { font-size: 11px; color: var(--text-3); }

  /* ── Preview ── */
  .pattern-preview { display: flex; flex-direction: column; align-items: center; gap: 16px; }
  .preview-name { font-size: 22px; font-weight: 700; letter-spacing: -0.5px; }
  .preview-info { font-size: 13px; color: var(--text-3); }
  ...
  .beat-blocks-active .beat-block { animation: pulse-block 0.6s ease-in-out infinite alternate; }
  ...
  .start-pattern-btn:hover { filter: brightness(1.1); transform: scale(1.02); }

  /* ── Round indicator ── */
  .round-row { display: flex; align-items: center; gap: 12px; }
  .round-text { font-size: 12px; color: var(--text-3); font-weight: 500; }
  .round-dots { display: flex; gap: 6px; }
  .round-dot {
    width: 8px; height: 8px; border-radius: 50%;
    border: 1.5px solid var(--text-3); transition: all 0.15s;
  }
  .round-dot.filled { background: var(--accent); border-color: var(--accent); }
  .round-dot.current { box-shadow: 0 0 8px var(--accent); }

  /* ── Countdown ── */
  .countdown-area { display: flex; flex-direction: column; align-items: center; gap: 8px; }
  .countdown-num {
    font-size: 72px; font-weight: 900; color: var(--text);
  ...
  .countdown-label { font-size: 13px; color: var(--text-3); }

  /* ── Playing ── */
  .playing-area { display: flex; flex-direction: column; align-items: center; gap: 16px; }
  .playing-pulse {
  ...
  .playing-label { font-size: 18px; font-weight: 700; color: var(--text); }

  /* ── Scoring ── */
  .scoring-area { display: flex; flex-direction: column; align-items: center; gap: 16px; width: 100%; max-width: 400px; }

  .complete-badge {
    padding: 6px 16px; border-radius: 8px;
    background: var(--green); color: white; font-size: 13px; font-weight: 600;
  }

  .score-display { display: flex; align-items: baseline; gap: 8px; }
  .score-value { font-size: 48px; font-weight: 900; letter-spacing: -2px; color: var(--text); }
  .score-label { font-size: 13px; color: var(--text-3); }

  /* ── Timing results ── */
  .timing-results { width: 100%; display: flex; flex-direction: column; gap: 4px; }
  .timing-row {
    display: flex; align-items: center; gap: 8px;
    padding: 6px 10px; border-radius: 6px; background: var(--surface-2);
    font-size: 13px;
  }
  .timing-label { flex: 1; color: var(--text-2); }
  .timing-icon { display: flex; align-items: center; }
  .timing-dev { font-size: 12px; font-weight: 600; min-width: 60px; text-align: right; font-variant-numeric: tabular-nums; }
  .timing-dev.good { color: var(--green); }
  .timing-dev.warn { color: var(--amber); }
  .timing-dev.miss { color: var(--red); }
  ...
  .trend-label { font-size: 10px; color: var(--text-3); text-transform: uppercase; letter-spacing: 1px; }
  .trend-values { display: flex; gap: 4px; font-size: 13px; font-weight: 600; }
  .trend-arrow { color: var(--text-3); margin: 0 2px; }

  /* ── Controls ── */
  .scoring-controls { display: flex; gap: 8px; margin-top: 8px; }
  .ctrl-btn {
  ...
  .ctrl-btn.accent { background: var(--accent); color: white; border-color: var(--accent); }
  .ctrl-btn.accent:hover { filter: brightness(1.1); }

  /* ── Sidebar task info ── */
  .pat-task-name { font-size: 16px; font-weight: 600; }
  .pat-task-meta { font-size: 12px; color: var(--text-3); }

  /* ── Mobile ── */
  @media (max-width: 768px) {
    .pattern-content { grid-template-columns: 1fr; }
    .pattern-sidebar { order: -1; }
  }
</style>
