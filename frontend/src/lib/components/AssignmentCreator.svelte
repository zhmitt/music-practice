...
<div class="creator-panel">
  ...
  <div class="creator-field">
    <label class="field-label">{$t('assignment.teacher_name')}</label>
    <input type="text" class="creator-input" bind:value={teacherName} placeholder={$t('assignment.teacher_placeholder')} />
  </div>

  <div class="creator-field">
    <label class="field-label">{$t('assignment.assignment_title')}</label>
    <input type="text" class="creator-input" bind:value={title} placeholder={$t('assignment.title_placeholder')} />
  </div>

  <div class="creator-field">
    <label class="field-label">{$t('assignment.description')}</label>
    <input type="text" class="creator-input" bind:value={description} placeholder={$t('assignment.desc_placeholder')} />
  </div>

  <div class="creator-field">
    <label class="field-label">{$t('assignment.due_date')}</label>
    <input type="date" class="creator-input" bind:value={dueDate} />
  </div>

  <div class="creator-field">
    <label class="field-label">{$t('assignment.notes')}</label>
    <input type="text" class="creator-input mono" bind:value={noteInput} placeholder="C4 D4 E4 F4 G4" />
    <span class="field-hint">{$t('assignment.notes_hint')}</span>
  </div>

  <div class="creator-field">
    <label class="field-label">{$t('assignment.hold_duration')}</label>
    <div class="dur-row">
      {#each [2, 3, 4, 6, 8] as d}
        <button class="dur-pill" class:active={duration === d} onclick={() => duration = d}>{d}s</button>
      {/each}
    </div>
  </div>

  <button class="create-btn" onclick={handleCreate} disabled={!title.trim() || !teacherName.trim() || parseNotes(noteInput).length === 0}>
...
<style>
  .creator-panel { display: flex; flex-direction: column; gap: 10px; }
  .creator-label { font-size: 10px; color: var(--text-3); text-transform: uppercase; letter-spacing: 1px; }

  .creator-field { display: flex; flex-direction: column; gap: 3px; }
  ...
  .creator-input::placeholder { color: var(--text-3); }
  .creator-input.mono { font-family: monospace; letter-spacing: 0.5px; }
  ...
  .dur-pill.active { background: var(--accent-soft); color: var(--accent); border-color: var(--accent); font-weight: 600; }
  .dur-pill:hover:not(.active) { background: var(--surface-hover); }
  ...
  .create-btn:hover:not(:disabled) { background: var(--accent); color: white; }
  .create-btn:disabled { opacity: 0.4; cursor: not-allowed; }

  .creator-flash { font-size: 11px; color: var(--green); font-weight: 600; }
</style>
