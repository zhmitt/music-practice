      ...
      {#each $importedPieces as piece (piece.id)}
        <div class="piece-item">
          <div class="piece-info">
            ...
            <div class="piece-meta">{piece.noteCount} {$t('import.notes')} &mdash; {piece.importedAt}</div>
          </div>
          <div class="piece-actions">
            ...
            <button class="piece-del-btn" onclick={() => removeImportedPiece(piece.id)} title="Delete">
              <svg viewBox="0 0 24 24" width="12" height="12">
...
<style>
  ...
  .import-title { font-size: 10px; color: var(--text-3); text-transform: uppercase; letter-spacing: 1px; }
  ...
  .import-btn:hover { background: var(--surface-hover); color: var(--text); }
  ...
  .piece-item:hover { border-color: var(--text-3); }

  .piece-info { flex: 1; min-width: 0; }
  .piece-title { font-size: 13px; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .piece-meta { font-size: 10px; color: var(--text-3); margin-top: 2px; }

  .piece-actions { display: flex; gap: 6px; margin-left: 12px; }

  .piece-play-btn, .piece-del-btn {
    width: 28px; height: 28px; border-radius: 6px; border: 1px solid var(--border);
    background: transparent; color: var(--text-3); cursor: pointer;
    display: flex; align-items: center; justify-content: center;
    transition: all 0.15s;
  }
  .piece-play-btn:hover { background: var(--accent-soft); color: var(--accent); border-color: var(--accent); }
  .piece-del-btn:hover { background: rgba(239,68,68,0.1); color: var(--red); border-color: var(--red); }

  .import-empty { font-size: 12px; color: var(--text-3); text-align: center; padding: 20px 0; margin: 0; }
</style>
