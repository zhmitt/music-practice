...
describe('Session store', () => {
  ...
  afterEach(async () => {
    ...
    try { await stopSession(); } catch { /* ok */ }
    vi.useRealTimers();
  });
  ...
  describe('startSession', () => {
    it('resets all state', async () => {
      ...
      toneResults.set([{ target: makeTone('X', 1), avgCents: 99, stability: 99, durationHeld: 99, centsSamples: [], passed: true }]);
  ...
  describe('repeatExercise', () => {
    it('resets tone index and results', async () => {
      await startFresh();
      toneIndex.set(1);
      toneResults.set([{ target: makeTone('Bb', 4), avgCents: 0, stability: 0, durationHeld: 5, centsSamples: [], passed: true }]);
      repeatExercise();
      expect(get(toneIndex)).toBe(0);
      expect(get(toneResults)).toEqual([]);
      expect(get(tonePhase)).toBe('waiting');
  ...
  describe('persistSessionResults', () => {
    it('computes accuracy as passed/total', async () => {
      await startFresh();
      toneResults.set([
        { target: makeTone('Bb', 4), avgCents: 2, stability: 1, durationHeld: 5, centsSamples: [], passed: true },
        { target: makeTone('C', 5), avgCents: 0, stability: 0, durationHeld: 0, centsSamples: [], passed: false },
      ]);
      await stopSession();
      const record = mockSaveSession.mock.calls[0][0];
    ...
    it('computes avgCents as mean of absolute values', async () => {
      await startFresh();
      toneResults.set([
        { target: makeTone('Bb', 4), avgCents: -8, stability: 1, durationHeld: 5, centsSamples: [], passed: true },
        { target: makeTone('C', 5), avgCents: 4, stability: 1, durationHeld: 5, centsSamples: [], passed: true },
      ]);
      await stopSession();
      const record = mockSaveSession.mock.calls[0][0];
    ...
    it('stores tone notes as "NoteName+Octave" strings', async () => {
      await startFresh();
      toneResults.set([
        { target: makeTone('Eb', 4), avgCents: 1, stability: 0.5, durationHeld: 5, centsSamples: [], passed: true },
      ]);
      await stopSession();
      const record = mockSaveSession.mock.calls[0][0];
